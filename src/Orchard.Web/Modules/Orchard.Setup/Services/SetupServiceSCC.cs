...
namespace Orchard.Setup.Services {
    public class SetupServiceSCC : ISetupService {
        ...
        public string Setup(SetupContext context) {
            string executionId;
            bool isOrchardAlreadyInitialized = false;
            ...
            using (var bootstrapLifetimeScope = _shellContainerFactory.CreateContainer(shellSettings, shellBlueprint)) {
                ...
                using (var environment = bootstrapLifetimeScope.CreateWorkContextScope()) {
                    ...
                    try {
                        ...
                        isOrchardAlreadyInitialized = true;
                    }
                    catch {
            ...
            using (var environment = _orchardHost.CreateStandaloneEnvironment(shellSettings)) {
                try {
                    if (isOrchardAlreadyInitialized)
                    {
                        // don't create default superuser second time, call recipe only
                        var recipeManager = environment.Resolve<IRecipeManager>();
                        executionId = recipeManager.Execute(Recipes().FirstOrDefault(r => r.Name.Equals(context.Recipe, StringComparison.OrdinalIgnoreCase)));
                    }
                    else
                    {
